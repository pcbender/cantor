...
def test_codex_cli_launch_is_supervised_and_records_provenance(tmp_path):
    ...
    assert "You are a Canto delegated Worker." in prompt
    assert "Read .canto/agents/shared.md and .canto/agents/executor.md" in prompt
    assert "public role is Worker" in prompt
    assert "Do not self-assign, broaden scope" in prompt
    assert "Do not modify the canonical repository" in prompt
    assert "immutable Result for Developer review" in prompt
